...
class MeshData {
    ...
    fixWinding() {
        if (!this.isReady())
            return; // If the mesh is not ready (i.e., indices array is empty or not divisible by 3), return
        for (let j = 0; j < this.indices.length; j += 3) {
            ...
            const cw = (vertB.x - vertA.x) * (vertC.y - vertA.y) -
     ...
     * ```
     */
    static createQuadMesh(size, uvBounds, cuts = new Vector2(6, 6), origin = new Vector2(0, 0)) {
        ...
        const m = new Map(); // Map to store vertex indices
        ...
        for (let y = 0; y < cuts.y + 1; y++) {
            if (!data.gridAxes)
                data.gridAxes = [[]]; // Initialize gridAxes if it doesn't exist
            data.gridAxes[0].push(y * sh - origin.y); // Add the y-coordinate to gridAxes[0]
            for (let x = 0; x < cuts.x + 1; x++) {
                if (!data.gridAxes[1])
                    data.gridAxes[1] = []; // Initialize gridAxes[1] if it doesn't exist
                ...
                const uv = new Vector2(uvBounds.x + x * uvx, // Calculate the x-coordinate of the UV
        ...
        // Generate indices
        const center = new Vector2(Math.floor(cuts.x / 2), // Calculate the x-coordinate of the center
        Math.floor(cuts.y / 2) // Calculate the y-coordinate of the center
        );
        for (let y = 0; y < cuts.y; y++) {
            for (let x = 0; x < cuts.x; x++) {
                const indice0 = [x, y]; // Bottom-left vertex index
                const indice1 = [x, y + 1]; // Top-left vertex index
                const indice2 = [x + 1, y]; // Bottom-right vertex index
                const indice3 = [x + 1, y + 1]; // Top-right vertex index
                ...
                if ((x < center.x && y < center.y) || (x >= center.x && y >= center.y)) {
                    data.indices.push(m.get(indice0), // Retrieve the bottom-left vertex index
     ...
     */
    isGrid() {
        return (this.gridAxes !== undefined && // Check if gridAxes is defined
     ...
     */
    clearGridIsDirty() {
        if (this.gridAxes === undefined || // Check if gridAxes is undefined
    ...
    regenerateGrid() {
        ...
        for (let i = 0; i < this.gridAxes[0].length - 1; i++) {
            ...
            for (let j = 0; j < this.gridAxes[1].length - 1; j++) {
                ...
                const indice3 = [x + 1, y + 1]; // Top-right vertex index
                // We want the vertices to generate in an X pattern so that we won't have too many distortion problems
                if ((xValue < center.x && yValue < center.y) || // Check if the vertex is in the bottom-left or top-right quadrant
                    (xValue >= center.x && yValue >= center.y) // Check if the vertex is in the bottom-right or top-left quadrant
                ) {
                    this.indices.push(m.get(indice0), // Retrieve the bottom-left vertex index
                    m.get(indice2), // Retrieve the bottom-right vertex index
                    m.get(indice3), // Retrieve the top-right vertex index
                    m.get(indice0), // Retrieve the bottom-left vertex index
                    m.get(indice3), // Retrieve the top-right vertex index
