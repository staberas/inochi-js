...
function deserializeBaseProperties(puppet, json, node) {
    ...
    node.children = json.children !== undefined ? json.children.map((child) => deserializeNode(puppet, child, node)) : node.children;
    node.lockToRoot = json.children !== undefined ? json.lockToRoot : node.lockToRoot;
    return node;
}
function deserializeDrawable(puppet, json, drawable) {
    drawable = deserializeBaseProperties(puppet, json, drawable);
    // Deserialize additional properties specific to Drawable
    drawable.mesh = MeshData.deserialize(json.mesh);
    // Populate the masks
    drawable.masks = json.masks !== undefined ? json.masks.map((mask) => {
...
function deserializePart(puppet, json) {
    let part = new Part();
    part = deserializeDrawable(puppet, json, part);
 ...
 */
export function deserializeNode(puppet, json, parent = null) {
    let result = new Node();
    switch (json.type) {
        case "Part":
